...
public class LegControl {
	...
	public LegControl(	BluetoothChatService bluetoothService, 
						Servo yaw, Servo pitch, 
						PointF pivot, PointF offset, 
						float defaultDistance, float minDistance, float maxDistance,
						double defaultAngle, double leftAngle, double rightAngle,
						float radius, float activeRadius, 
						Paint paint, Paint activePaint, Paint arcPaint) {
		super();
	...
	public boolean move(PointF destination)
	{
		...
		if((int)position.y != (int)newY || (int)position.x != (int)newX)
		{
			...
			int newYawAngle = (int)Math.toDegrees(rotate(testAngle, Math.PI/2));
			
			int newPitchAngle = 0;
			if(relativeMagnitude > defaultDistance*scale)
			{
				newPitchAngle = (int)(90 / (maxDistance*scale - defaultDistance*scale) * (relativeMagnitude - defaultDistance*scale) + 90);
			}
			else
			{
				newPitchAngle = (int)(90 / (defaultDistance*scale - minDistance*scale) * (relativeMagnitude- minDistance*scale));
			}
	        ...
	        {  
                if(newYawAngle != yaw.getAngle())
                {
                    byte[] yawMsg = yaw.rotateTo(newYawAngle).getBytes();
                	bluetoothService.write(yawMsg);
                }
                
                if(newPitchAngle != pitch.getAngle())
                {
                    byte[] pitchMsg = pitch.rotateTo(newPitchAngle).getBytes();
                	bluetoothService.write(pitchMsg);
                }
	        }
		}
